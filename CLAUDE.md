...
**Sealbox** - Cross-platform Go library (`package sealbox`) for TPM 2.0 (Trusted Platform Module) key sealing and unsealing operations. Hardware-bound encryption where sealed keys cannot be extracted or used on other machines.
...
- **KeyStore** (`sealbox.go`): Filesystem persistence with platform-specific default paths using functional options pattern
...
- **Windows ACLs** (`sealbox_windows.go`): Proper Windows file permissions using internal acl package
...
| `sealbox.go` | File-based key storage |
